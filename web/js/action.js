...
function tableBody(currentPage1) {
    ...
    for (let i = starIndex; i < endIndex; i++) {
        ...
        for (let j = 0; j < headDate.length + 1; j++) {
            ...
            if (j === 0) {
                ...
                let buttonAll1 = $("<button/>").addClass("iconfont icon-sousuo").prop("id", 2 * i);
                let buttonAll2 = $("<button/>").addClass("iconfont icon-write").prop("id", 2 * i + 1);
    ...
    let pageDate = $("#pageDate").text("第" + currentPage + "页，共" + allTr + "条，(每页显示" + pageMaxTr + "条)");
}
